...
import { FILES_SESSION_DATA } from "./files.constant";
...
export default function useFiles() {
  ...
  const filterData = () => {
    const filteredData = FILES_SESSION_DATA.filter((item) => {
      let engineFlag = false;
      let userFlag = false;
      let textFlag = false;
  
      if (engineName.length === 0 || engineName.includes(item.intraction.split(" ")[0])) {
        engineFlag = true;
      }
  
      if (userName === "" || item.intraction === userName) {
        userFlag = true;
      }
